...
(function() {
  ...
  $('bb-save-key')?.addEventListener('click', () => {
    ...
    if (!key.startsWith('gsk_')) {
      $('bb-key-status').innerHTML = '<span style="color:#c97b5e;">⚠ Key should start with "gsk_". Double-check the copy.</span>';
      return;
    }
    localStorage.setItem(STORAGE_KEY, key);
    $('bb-key-status').innerHTML = '<span style="color:#5a7a5e;">✓ Key saved! Click Generate to try it.</span>';
    $('bb-prompt').focus();
  });
